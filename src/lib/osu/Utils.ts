...
export const AddButtons = ({ Name, Flags: { m, acc, b, g, l, map, mods, offset, p, rand, rv, includeFail } }: Args, scoreCount: number, callback: iOnButton, increment=5) => {
    const buttons = []
    
    if (offset !== 0) buttons.push(AddButton("⬅️", { Name, Flags: { m, acc, b, g, l, map, mods, offset: offset - 5, p, rand, rv, includeFail: includeFail ?? true } }, callback))
    if (offset + increment < scoreCount) buttons.push(AddButton("➡️", { Name, Flags: { m, acc, b, g, l, map, mods, offset: offset + increment, p, rand, rv, includeFail: includeFail ?? true } }, callback))
